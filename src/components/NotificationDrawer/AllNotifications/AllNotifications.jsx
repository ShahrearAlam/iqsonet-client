...
export default function AllNotifications() {
    ...
    return (
        <div>
            {
                notifications.map(({ _id, type, post, message, user: { _id: userId, personal: { fullname }, username }, readingStatus, createdAt }) => <div onClick={() => { handleNotificationRead(_id, readingStatus, type, userId, post) }} key={_id} className={`flex items-start gap-2  py-7 px-1 rounded-sm border-b relative cursor-pointer ${theme ? "hover:bg-gray-900 border-b-gray-700" : "hover:bg-gray-50 border-b-gray-200"}`}>
                    ...
                    {
                        readingStatus === "unread" && <div className=" h-2 w-2 rounded-full bg-sky-500 absolute  right-4 top-4"></div>
                    }
                </div>)
            }
        </div>
